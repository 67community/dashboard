...
export async function GET() {
  ...
  const out = {
    ...
    tiktok_spotlight:    Array.isArray(sbTikTok) ? sbTikTok : ((sbTikTok as any)?.videos ?? static_?.tiktok_spotlight ?? []),
    youtube_spotlight:   (sbYouTube    as unknown[]) ?? static_?.youtube_spotlight    ?? [],
    youtube_analytics:   static_?.youtube_analytics  ?? null,
    instagram_spotlight: Array.isArray(sbInstagram) ? ((sbInstagram as any)?.posts ?? sbInstagram) : ((sbInstagram as any)?.posts ?? static_?.instagram_spotlight ?? []),
