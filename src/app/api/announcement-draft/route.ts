...
Return ONLY the announcement text, nothing else.`,
    ...
    return NextResponse.json({ body: result.text, _provider: result.provider })
  } catch {
    const mock = (MOCKS[type] ?? MOCKS.general).replace("{{title}}", title ?? "")
    return NextResponse.json({ body: mock })
  }
}
