...
const PRIORITY_STYLES: Record<Priority, string> = {
  Low: "bg-[#F4F4F5] text-[#71717A] border-[rgba(0,0,0,0.07)]",
      ...
      <div className="relative w-full sm:max-w-2xl max-h-[90vh] bg-white rounded-t-3xl sm:rounded-3xl shadow-2xl overflow-hidden flex flex-col animate-in slide-in-from-bottom-4 duration-300">
        ...
        <div className="flex items-start justify-between px-6 py-5 border-b border-[rgba(0,0,0,0.05)] flex-shrink-0">
          <div className="flex-1 pr-4">
            <input
              className="w-full text-lg font-bold text-[#09090B] bg-transparent border-none outline-none focus:ring-0 resize-none"
          ...
          <button
            onClick={onClose}
            className="w-8 h-8 rounded-full bg-[#F4F4F5] hover:bg-[#E8E8EA] flex items-center justify-center transition-colors flex-shrink-0"
          >
        ...
        <div className="overflow-y-auto flex-1 p-6 space-y-5">
          ...
          <div className="flex flex-wrap gap-2">
            ...
            <select
              ...
              className="text-xs px-3 py-1.5 bg-[#F4F4F5] rounded-xl border-none outline-none cursor-pointer font-medium text-[#09090B]"
            >
          ...
          <div className="flex items-center gap-4">
            <div className="flex items-center gap-2">
              ...
              <select
                ...
                className="text-xs bg-transparent border-none outline-none cursor-pointer text-[#71717A]"
              >
          ...
          {/* Description */}
          <div>
            <p className="text-xs font-semibold text-[#A1A1AA] uppercase tracking-wider mb-2">Description</p>
            <textarea
              className="w-full min-h-[80px] text-sm text-[#09090B] bg-[#F4F4F5] rounded-xl p-3 border border-[rgba(0,0,0,0.06)] outline-none focus:border-amber-200 focus:ring-1 focus:ring-amber-200 resize-none transition-colors"
          ...
          <div>
              ...
              {localTask.subtasks.map((st) => (
                ...
                >
                  {st.done ? (
                    ...
                    <Square className="w-4 h-4 text-gray-300 group-hover:text-[#A1A1AA] flex-shrink-0" />
                  )}
                  <span className={`text-sm ${st.done ? "line-through text-[#A1A1AA]" : "text-[#09090B]"}`}>
                    {st.title}
                  </span>
                </div>
              ))}
            ...
            <div className="flex items-center gap-2 mt-2">
              <input
                className="flex-1 text-sm bg-gray-50 rounded-xl px-3 py-2 border border-gray-100 outline-none focus:border-amber-200 focus:ring-1 focus:ring-amber-200 transition-colors placeholder:text-[#A1A1AA]"
          ...
          {localTask.activity.length > 0 && (
            <div>
              <p className="text-xs font-semibold text-[#A1A1AA] uppercase tracking-wider mb-2">Activity</p>
              <div className="space-y-1.5">
                {localTask.activity.map((a) => {
                  const actor = TEAM_MEMBERS.find((m) => m.id === a.actorId)
                  return (
                    <div key={a.id} className="flex items-center gap-2 text-xs text-[#71717A]">
                      ...
                      <span className="font-medium text-[#71717A]">{actor?.name || a.actorId}</span>
                      <span>{a.action}</span>
                      <span className="text-[#A1A1AA]">{new Date(a.timestamp).toLocaleDateString()}</span>
                    </div>
                  )
                })}
              </div>
