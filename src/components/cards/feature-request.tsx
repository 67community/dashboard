  ...
  return (
    <div style={{ display:"flex", flexDirection:"column", gap:10 }}>
      ...
      <form onSubmit={submit} onClick={e => e.stopPropagation()} style={{ display:"flex", flexDirection:"column", gap:6 }}>
        <textarea ref={el => { if(el){el.style.height="auto";el.style.height=Math.max(32,el.scrollHeight)+"px"} }} value={what} onChange={e => { setWhat(e.target.value); const t=e.target; t.style.height="auto"; t.style.height=Math.max(32,t.scrollHeight)+"px" }} placeholder="What to build?" required rows={1}
          style={{ width:"100%", boxSizing:"border-box", padding:"7px 10px", borderRadius:8, border:"1.5px solid var(--separator)", fontSize:"0.75rem", fontFamily:"inherit", background:"var(--input-bg)", color:"var(--foreground)", outline:"none", resize:"none", overflow:"hidden", minHeight:32 }}
          onFocus={e => e.currentTarget.style.borderColor="#F5A623"} onBlur={e => e.currentTarget.style.borderColor="var(--separator)"} />
        <textarea ref={el => { if(el){el.style.height="auto";el.style.height=Math.max(32,el.scrollHeight)+"px"} }} value={why} onChange={e => { setWhy(e.target.value); const t=e.target; t.style.height="auto"; t.style.height=Math.max(32,t.scrollHeight)+"px" }} placeholder="Why? (value / problem)" required rows={1}
          style={{ width:"100%", boxSizing:"border-box", padding:"7px 10px", borderRadius:8, border:"1.5px solid var(--separator)", fontSize:"0.75rem", fontFamily:"inherit", background:"var(--input-bg)", color:"var(--foreground)", outline:"none", resize:"none", overflow:"hidden", minHeight:32 }}
          onFocus={e => e.currentTarget.style.borderColor="#F5A623"} onBlur={e => e.currentTarget.style.borderColor="var(--separator)"} />
