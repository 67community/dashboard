...
type Tab = "tiktok" | "youtube" | "instagram"
  ...
  const collapsed = (
    <div style={{ display:"flex", flexDirection:"column", gap:16 }}>
      <div style={{ display:"flex", gap:10 }}>
        <div style={{ flex:1, background:"rgba(0,0,0,0.04)", borderRadius:10, padding:"10px 12px" }}>
          <div style={{ display:"flex", alignItems:"center", gap:5, marginBottom:3 }}>
            <TikTokIcon size={12} /><span style={{ fontSize:"0.5625rem", fontWeight:700, color:"var(--secondary)", textTransform:"uppercase", letterSpacing:"0.06em" }}>TikTok</span>
          </div>
          <p style={{ fontSize:"1.25rem", fontWeight:800, color:"var(--foreground)", letterSpacing:"-0.03em", margin:0 }}>{fmt(ttViews)}</p>
          <p style={{ fontSize:"0.5625rem", color:"var(--tertiary)", margin:"2px 0 0" }}>combined views</p>
        </div>
        <div style={{ flex:1, background:"rgba(255,0,0,0.05)", borderRadius:10, padding:"10px 12px" }}>
          <div style={{ display:"flex", alignItems:"center", gap:5, marginBottom:3 }}>
            <YouTubeIcon size={12} /><span style={{ fontSize:"0.5625rem", fontWeight:700, color:"var(--secondary)", textTransform:"uppercase", letterSpacing:"0.06em" }}>YouTube</span>
          </div>
          <p style={{ fontSize:"1.25rem", fontWeight:800, color:"var(--foreground)", letterSpacing:"-0.03em", margin:0 }}>{fmt(ytViews)}</p>
          <p style={{ fontSize:"0.5625rem", color:"var(--tertiary)", margin:"2px 0 0" }}>combined views</p>
        </div>
        <div style={{ flex:1, background:"rgba(225,48,108,0.05)", borderRadius:10, padding:"10px 12px" }}>
          <div style={{ display:"flex", alignItems:"center", gap:5, marginBottom:3 }}>
            <InstaIcon size={12} /><span style={{ fontSize:"0.5625rem", fontWeight:700, color:"var(--secondary)", textTransform:"uppercase", letterSpacing:"0.06em" }}>Instagram</span>
          </div>
          <p style={{ fontSize:"1.25rem", fontWeight:800, color:"var(--foreground)", letterSpacing:"-0.03em", margin:0 }}>{igLikes > 0 ? fmt(igLikes) : "—"}</p>
          <p style={{ fontSize:"0.5625rem", color:"var(--tertiary)", margin:"2px 0 0" }}>combined likes</p>
        </div>
      </div>

      <div style={{ display:"flex", gap:8 }}>
        {tiktoks.slice(0,2).map((v, i) => (
          <a key={i} href={v.video_url} target="_blank" rel="noopener noreferrer" style={{ flex:1, textDecoration:"none" }}>
            <div style={{ position:"relative", borderRadius:8, overflow:"hidden", aspectRatio:"9/16", background:"#000", maxHeight:130 }}>
              ...
              <div style={{ position:"absolute", bottom:5, left:5, display:"flex", alignItems:"center", gap:3 }}>
                <TikTokIcon size={9} /><span style={{ fontSize:"0.5rem", color:"white", fontWeight:700 }}>{fmt(v.plays||0)}</span>
              </div>
            </div>
          </a>
        ))}
        {youtubes.slice(0,1).map((v: any, i) => (
          <a key={i} href={v.url||"#"} target="_blank" rel="noopener noreferrer" style={{ flex:1, textDecoration:"none" }}>
            <div style={{ position:"relative", borderRadius:8, overflow:"hidden", background:"#000", maxHeight:130, height:"100%" }}>
              {v.thumbnail && <img src={v.thumbnail} alt="" style={{ width:"100%", height:"100%", objectFit:"cover" }} />}
              <div style={{ position:"absolute", inset:0, background:"rgba(0,0,0,0.3)", display:"flex", alignItems:"center", justifyContent:"center" }}>
                <div style={{ width:24, height:24, background:"rgba(255,0,0,0.9)", borderRadius:5, display:"flex", alignItems:"center", justifyContent:"center" }}>
                  <Play size={10} color="white" fill="white" />
                </div>
              </div>
              <div style={{ position:"absolute", bottom:5, left:5, display:"flex", alignItems:"center", gap:3 }}>
                <YouTubeIcon size={9} /><span style={{ fontSize:"0.5rem", color:"white", fontWeight:700 }}>{fmt(v.views||0)}</span>
              </div>
            </div>
          </a>
        ))}
