      ...
      <div className="grid grid-cols-2 gap-2.5">
        {[
          ...
          <div key={x.l} className="stat-pill">
            ...
            <p className="text-base font-bold text-[#1D1D1F] tracking-tight">{x.v}</p>
          </div>
        ))}
      </div>
    </div>
  ...
  const expanded = (
    <div className="space-y-5">
      ...
      <div className="bg-[#F5F5F7] rounded-2xl p-5">
        ...
        <p style={{ fontSize: "3rem", fontWeight: 800, letterSpacing: "-0.05em", lineHeight: 1, color: "#1D1D1F" }}>
          {$p(price)}
        </p>
        <div className="flex items-center gap-2 mt-2.5">
          ...
          <span className="text-xs text-[#8E8E93] font-medium">1h</span>
          {t?.price_change_7d !== undefined && <>
            <span className="text-xs text-[#8E8E93]">·</span>
            <Chg v={t.price_change_7d} />
            <span className="text-xs text-[#8E8E93] font-medium">7d</span>
          </>}
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 gap-2.5">
        {[
          ...
          <div key={x.l} className="stat-pill">
            <p className="display-label mb-1.5">{x.l}</p>
            <p className="text-xl font-bold text-[#1D1D1F] tracking-tight">{x.v}</p>
          </div>
        ))}
      </div>

      {/* ATH */}
      <div className="rounded-2xl p-5 overflow-hidden relative" style={{ background: "linear-gradient(135deg, #1D1D1F, #2D2D2F)" }}>
            ...
            {t!.exchange_volumes.map((ex, i) => {
              ...
              return (
                <div key={i} className="flex items-center gap-2.5">
                  ...
                  <span className="text-xs font-semibold text-[#1D1D1F] w-28 truncate">{ex.exchange}</span>
                  ...
                  <span className="text-xs font-bold text-[#1D1D1F] w-14 text-right">{$(ex.volume_usd)}</span>
                </div>
              )
            })}
          </div>
      ...
      <div className="flex flex-wrap gap-2">
        {[["DexScreener","https://dexscreener.com/solana/DMAFl613xtipUA3JFNycZaVwT7XsIYf9CR3QmrmZqhB6"],
          ...
          <a key={l} href={h} target="_blank" rel="noopener noreferrer" onClick={e=>e.stopPropagation()}
            className="px-3.5 py-2 rounded-xl text-xs font-bold text-[#1D1D1F] hover:bg-[#E5E5EA] transition-colors"
