      ...
      <div className="space-y-2.5">
        {bots.slice(0, 5).map(b => (
          <div key={b.name} className="flex items-center gap-2.5">
            ...
            <span className="text-xs font-semibold text-[#3C3C43] flex-1">{b.name}</span>
            <span className="text-xs text-[#C7C7CC] font-medium">{ago(b.last_run)}</span>
          </div>
        ))}
      </div>
    </div>
  ...
  const expanded = (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 gap-2.5">
        ...
        <div className="rounded-2xl p-4" style={{ background: bots.length - on > 0 ? "#FEF2F2" : "#F5F5F7" }}>
          <p className="text-3xl font-black tracking-tight" style={{ color: bots.length - on > 0 ? "#DC2626" : "#C7C7CC" }}>
            {bots.length - on}
          </p>
          <p className="text-xs font-bold mt-0.5 tracking-widest uppercase" style={{ color: bots.length - on > 0 ? "#DC2626" : "#C7C7CC", opacity: 0.7 }}>
            Offline
          </p>
        </div>
      </div>
      ...
      {bots.map(b => (
        <div key={b.name} className="flex items-center gap-3.5 stat-pill">
          ...
          <div className="flex-1">
            <p className="text-sm font-bold text-[#1D1D1F]">{b.name}</p>
            <p className="text-xs text-[#8E8E93]">{b.schedule}</p>
          </div>
          <div className="text-right">
            <p className="text-xs font-bold" style={{ color: b.status === "green" ? "#34C759" : "#FF3B30" }}>
              {b.status === "green" ? "Running" : "Offline"}
            </p>
            <p className="text-xs text-[#C7C7CC]">{ago(b.last_run)} ago</p>
          </div>
        </div>
      ))}
    </div>
