    ...
    <div className="space-y-5">
      ...
      {s?.best_tweet_week && (
        <div className="rounded-2xl p-4" style={{ background: "#F5F5F7" }}>
          <p className="text-xs text-[#8E8E93] line-clamp-2 leading-relaxed mb-2.5">
            {s.best_tweet_week.text.slice(0, 120)}…
          </p>
          <div className="flex items-center gap-3 text-xs font-bold text-[#8E8E93]">
            ...
            <span className="ml-auto text-[#C7C7CC]">{s.best_tweet_week.date}</span>
          </div>
        </div>
      )}
    </div>
    ...
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-3 gap-2.5">
        <div className="stat-pill">
          ...
          <p className="text-2xl font-black text-[#1D1D1F] tracking-tight">{fmtFollowers}</p>
        </div>
        <div className="stat-pill">
          ...
          <div className="space-y-2">
            {Object.entries(s.content_type_stats).map(([type, st]) => (
              <div key={type} className="flex items-center justify-between">
                <span className="text-xs font-semibold text-[#3C3C43] capitalize w-32">{type.replace("_"," ")}</span>
                <span className="text-xs text-[#8E8E93]">{st.count} posts</span>
                <span className="text-xs font-bold text-[#1D1D1F]">{st.avg_eng.toFixed(0)} avg</span>
              </div>
            ))}
          </div>
        </div>
        ...
        <div>
          ...
          {s.best_tweet_week.embed_html
            ...
            : <a href={s.best_tweet_week.tweet_url} target="_blank" rel="noopener noreferrer" onClick={e=>e.stopPropagation()}
                className="block stat-pill hover:bg-[#EBEBF0] transition-colors">
                <p className="text-sm text-[#1D1D1F] leading-relaxed">{s.best_tweet_week.text}</p>
                <div className="flex gap-4 mt-3 text-xs font-bold text-[#8E8E93]">
        ...
        <div>
          ...
          {s.best_tweet_2d.embed_html
            ...
            : <a href={s.best_tweet_2d.tweet_url} target="_blank" rel="noopener noreferrer" onClick={e=>e.stopPropagation()}
                className="block stat-pill hover:bg-[#EBEBF0] transition-colors">
                <p className="text-sm text-[#1D1D1F] leading-relaxed">{s.best_tweet_2d.text}</p>
                <div className="flex gap-4 mt-3 text-xs font-bold text-[#8E8E93]">
